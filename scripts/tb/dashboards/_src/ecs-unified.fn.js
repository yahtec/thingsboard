...
function PACV2_FLATTEN(p) {
    ...
    if (p.heat) for (var __k in p.heat) {
        var __v = p.heat[__k];
        if (__v === null || __v === undefined) continue;
        // (heat.calo non flattene ici : page ECS, pas de calorimetre chauffage)
        if (typeof __v !== 'object') out['heat_' + __k] = __v;
    }
...
function wireTimelineBar(){
  ...
  function refetchNow(){
    if (window.__tbEcsUnified && typeof window.__tbEcsUnified.refetch === 'function') window.__tbEcsUnified.refetch();
  }
  Array.prototype.forEach.call(btns, function(b){
...
var html = '<div class="u-root">'+
  ...
  '<div class="u-scroll">'+
    ...
    '<div class="u-grid" id="u-pumps" style="display:grid;grid-template-columns:repeat(4, 1fr);gap:12px;margin-top:12px"></div>'+
  '</div></div>';
...
function wireResponsiveGrid(){
  ...
  function apply(){ var w = (document.querySelector('.u-scroll')||{}).clientWidth || 1000;
    var cols = w < 600 ? '1fr' : (w < 850 ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)');
    grids.forEach(function(g){ g.style.gridTemplateColumns = cols; }); if(window.__tbEcsUnified.onResize) window.__tbEcsUnified.onResize(); }
  apply();
  ...
  window.__tbEcsUnified.ro = ro;
}
setTimeout(function(){
  var prev = window.__tbEcsUnified || {};
  ...
  window.__tbEcsUnified = { vis:{}, listeners:{} };
  ...
  if (!window.__tbEcsRetroWired) {
    window.__tbEcsRetroWired = true;
    window.addEventListener('tduo:retroview', function(){
      if (window.__tbEcsUnified && typeof window.__tbEcsUnified.refetchAll === 'function') window.__tbEcsUnified.refetchAll();
      else if (window.__tbEcsUnified && typeof window.__tbEcsUnified.refetch === 'function') window.__tbEcsUnified.refetch();
...
function renderChart(cfg, seriesData, win, evtIntervals) {
    ...
    window.__tbEcsUnified.vis[cfg.id] = window.__tbEcsUnified.vis[cfg.id] || {};
    var visibility = window.__tbEcsUnified.vis[cfg.id];
    ...
    var L = window.__tbEcsUnified.listeners = window.__tbEcsUnified.listeners || {};
...
// =====================================================================================
// [H] tableaux pompes (pumpsInfo) + helpers partages isBad/fv/tH
// =====================================================================================
function isBad(val) {
