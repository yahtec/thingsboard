...
function buildGauge(id,label,value,tempLabel,tempValue,min,max,color,colorLight){
  ...
  return '<div style="display:flex;flex-direction:column;align-items:stretch;background:#fff;border:1px solid #e0e0e0;border-radius:6px;padding:8px;box-sizing:border-box;flex:0 0 auto;width:270px">'+
    ...
    '<div style="display:flex;flex-direction:column;align-items:center;gap:6px">'+
      '<svg viewBox="0 0 200 200" preserveAspectRatio="xMidYMid meet" style="width:100%;max-width:255px;height:auto;display:block;margin:0 auto">'+
...
// Porte de usage_pie.controller.js. Tout est scope dans cette IIFE pour eviter toute
// collision avec les helpers top-level (render/fv/isBad/fmtDuration...). Le donut a sa
// propre plage temporelle (boutons presets), independante de la boucle 30s des charts.
// =====================================================================================
(function(){
  var U_COLORS = { pac:'#1976d2', boil:'#e53935', off:'#bdbdbd' };
  var U_LABELS = { pac:'PAC seule', boil:'Chaudière seule', off:'Arrêt' };
  var U_SLICE_ORDER = ['pac','boil','off'];
  var U_PRESETS = [
  ...
  var U_LS_KEY = 'tduo.usagePie.range';
  ...
  function loadRange() {
    try {
      var raw = localStorage.getItem(U_LS_KEY);
      if (raw) {
        ...
        if (p && p.preset) {
          var preset = U_PRESETS.filter(function(x){ return x.id === p.preset; })[0];
          if (preset) { var end = Date.now(); return { preset:p.preset, start:end - preset.ms, end:end }; }
        }
      }
    } catch(_) {}
    var end = Date.now();
    var defP = U_PRESETS.filter(function(x){ return x.id === '30j'; })[0];
  ...
  function saveRange() {
    try {
      ...
      localStorage.setItem(U_LS_KEY, JSON.stringify(p));
    } catch(_) {}
  }
  ...
  function renderSvg(buckets, sum) {
    ...
    wrap.style.cssText = 'display:flex;justify-content:center;align-items:center;padding:6px 0';
    ...
    var nonZero = U_SLICE_ORDER.filter(function(k){ return buckets[k] > 0; });
    if (nonZero.length === 1) {
      var only = document.createElementNS(ns, 'circle');
      only.setAttribute('cx', cx); only.setAttribute('cy', cy); only.setAttribute('r', r);
      only.setAttribute('fill', U_COLORS[nonZero[0]]);
      svg.appendChild(only);
    } else {
      var start = -Math.PI / 2;
      U_SLICE_ORDER.forEach(function(k) {
        var v = buckets[k];
        if (v <= 0) return;
        var angle = (v / sum) * Math.PI * 2;
        var endA = start + angle;
        var p = document.createElementNS(ns, 'path');
        p.setAttribute('d', arcPath(cx, cy, r, start, endA));
        p.setAttribute('fill', U_COLORS[k]);
        svg.appendChild(p);
        start = endA;
      });
    }
    ...
    t1.setAttribute('font-size', '22'); t1.setAttribute('font-weight', '700'); t1.setAttribute('fill', '#333');
    t1.textContent = pctActive + '%';
    svg.appendChild(t1);
    var t2 = document.createElementNS(ns, 'text');
    t2.setAttribute('x', cx); t2.setAttribute('y', cy + 13);
    t2.setAttribute('text-anchor', 'middle');
    t2.setAttribute('font-size', '10'); t2.setAttribute('fill', '#888');
  ...
  function renderLegend(buckets, sum) {
    ...
    U_SLICE_ORDER.forEach(function(k) {
      ...
      row.style.cssText = 'display:flex;align-items:center;gap:8px;font-size:13px;padding:3px 0;border-bottom:1px solid #f0f0f0';
      row.innerHTML =
        '<span style="display:inline-block;width:12px;height:12px;border-radius:2px;flex:0 0 auto;background:' + U_COLORS[k] + '"></span>' +
        '<span style="flex:1 1 auto;color:#444">' + U_LABELS[k] + '</span>' +
        '<span style="font-weight:700;color:#333;min-width:48px;text-align:right">' + (pct < 0.05 ? '0' : pct.toFixed(1)) + '%</span>' +
        '<span style="color:#888;min-width:64px;text-align:right">' + fmtDurUsage(ms) + '</span>';
      wrap.appendChild(row);
    });
    return wrap;
  }
  function renderUsageDonut(body, buckets, meta) {
    var sum = buckets.pac + buckets.boil + buckets.off;
    if (sum <= 0) {
      body.innerHTML = '<div style="padding:16px;text-align:center;color:#888;font-size:13px">Aucune donnée sur la période.</div>';
      return;
    }
    body.innerHTML = '';
    body.appendChild(renderSvg(buckets, sum));
    body.appendChild(renderLegend(buckets, sum));
    ...
    foot.style.cssText = 'margin-top:8px;font-size:10px;color:#aaa;text-align:center';
    foot.textContent = 'PAC/Chaud = compteurs pac_v2 (unité ' + unitTxt + ') · périmètre : ' + scope + '.';
    body.appendChild(foot);
  }

  function refresh() {
    var body = document.getElementById('u-usage-body');
    if (!body) return;
    body.innerHTML = '<div style="padding:16px;text-align:center;color:#888;font-size:13px">Chargement…</div>';

    var devId = resolveDevice();
    if (!devId) {
      body.innerHTML = '<div style="padding:16px;text-align:center;color:#888;font-size:13px">Aucune chaufferie sélectionnée.</div>';
      return;
    }
    var r = U.range || { start: Date.now() - 30*86400*1000, end: Date.now() };
    if (r.preset && r.preset !== 'custom') {
      var p = U_PRESETS.filter(function(x){ return x.id === r.preset; })[0];
      if (p) { r.end = Date.now(); r.start = r.end - p.ms; }
    }
    var start = r.start, end = r.end;
    var periodEl = document.getElementById('u-usage-period');
    if (periodEl) periodEl.textContent = fmtPeriod(start, end);

    var hpIndex = P;
    var headers = { 'X-Authorization': 'Bearer ' + getToken() };
    ...
    Promise.all([
      ...
      if (!prePoint || !endPoint) {
        body.innerHTML = '<div style="padding:16px;text-align:center;color:#888;font-size:13px">Pas de données pac_v2 sur la période.</div>';
        return;
      }
      var preP = parsePacV2(prePoint.value);
      var endP = parsePacV2(endPoint.value);
      if (!preP || !endP) {
        body.innerHTML = '<div style="padding:16px;text-align:center;color:#888;font-size:13px">Données pac_v2 invalides.</div>';
        return;
      }
      ...
      renderUsageDonut(body, { pac: totalPacMs, boil: totalBoilMs, off: off }, {
        wallclockMs: wallclockMs, unitDetected: mode,
        hpIndex: hpIndex, activeHPs: activeHPs
      });
    }).catch(function(err) {
      body.innerHTML = '<div style="padding:16px;text-align:center;color:#888;font-size:13px">Erreur: ' + (err && err.message ? err.message : err) + '</div>';
    });
  }

  function applyPresetButtons(host) {
    host.querySelectorAll('[data-preset]').forEach(function(btn) {
      var active = btn.getAttribute('data-preset') === U.range.preset;
      btn.style.cssText = 'cursor:pointer;font-size:12px;padding:4px 10px;border-radius:4px;border:1px solid ' +
        (active ? '#1976d2' : '#ccc') + ';background:' + (active ? '#1976d2' : '#fff') +
        ';color:' + (active ? '#fff' : '#555') + ';';
    });
  }
  function wirePresets(host) {
    host.querySelectorAll('[data-preset]').forEach(function(btn) {
      btn.addEventListener('click', function() {
        var id = btn.getAttribute('data-preset');
        if (id === 'custom') {
          var pane = document.getElementById('u-usage-custom');
          var open = pane.style.display === 'none' || !pane.style.display;
          pane.style.display = open ? 'flex' : 'none';
          if (open) {
            document.getElementById('u-usage-from').value = ymd(U.range.start);
            document.getElementById('u-usage-to').value   = ymd(U.range.end);
          }
          return;
        }
        var preset = U_PRESETS.filter(function(x){ return x.id === id; })[0];
        if (!preset) return;
        var end = Date.now();
        U.range = { preset:id, start:end - preset.ms, end:end };
        saveRange();
        document.getElementById('u-usage-custom').style.display = 'none';
        applyPresetButtons(host);
        refresh();
      });
    });
    var apply = host.querySelector('#u-usage-apply');
    if (apply) apply.addEventListener('click', function() {
      var fromV = document.getElementById('u-usage-from').value;
      var toV   = document.getElementById('u-usage-to').value;
      if (!fromV || !toV) return;
      var s = new Date(fromV + 'T00:00:00').getTime();
      var e = new Date(toV   + 'T23:59:59').getTime();
      if (!(s < e)) return;
      U.range = { preset:'custom', start:s, end:e };
      saveRange();
      applyPresetButtons(host);
      refresh();
    });
  ...
  function renderWidget(host) {
    var presetsHtml = U_PRESETS.map(function(p) {
      return '<button type="button" data-preset="' + p.id + '">' + p.label + '</button>';
    }).join('') + '<button type="button" data-preset="custom">Personnalisé</button>';
    var title = P ? ('Utilisation PAC ' + P) : "Répartition d'utilisation";
    host.innerHTML =
      '<div style="max-width:520px;margin:0 auto;background:#fff;border:1px solid #e0e0e0;border-radius:8px;padding:14px;box-sizing:border-box">' +
        '<div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px">' +
          '<span style="font-size:16px;font-weight:700;color:#333;text-transform:uppercase;letter-spacing:1px">' + title + '</span>' +
          '<span id="u-usage-period" style="font-size:11px;color:#888">—</span>' +
        '</div>' +
        '<div id="u-usage-presets" style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px">' + presetsHtml + '</div>' +
        '<div id="u-usage-custom" style="display:none;align-items:center;gap:6px;margin-bottom:8px;font-size:12px;color:#555">' +
          '<label>Du</label><input type="date" id="u-usage-from" style="font-size:12px">' +
          '<label>au</label><input type="date" id="u-usage-to" style="font-size:12px">' +
          '<button type="button" id="u-usage-apply" style="cursor:pointer;font-size:12px;padding:4px 10px;border-radius:4px;border:1px solid #1976d2;background:#1976d2;color:#fff">Appliquer</button>' +
        '</div>' +
        '<div id="u-usage-body"><div style="padding:16px;text-align:center;color:#888;font-size:13px">Chargement…</div></div>' +
      '</div>';
    U.range = loadRange();
    wirePresets(host);
    applyPresetButtons(host);
  ...
  window.__renderUsage = function() {
    var host = document.getElementById('u-usage');
    if (!host) return;

    // Mode retroview : compteurs cumules incompatibles avec rejouage -> note + sortie.
    try {
      var __rv = sessionStorage.getItem('tduo.retroview.endTs');
      if (__rv) {
        host.innerHTML = '<div style="max-width:520px;margin:0 auto;background:#fff;border:1px solid #e0e0e0;border-radius:8px;padding:16px;text-align:center;color:#888;font-size:13px">Répartition d\'utilisation indisponible en mode rétroview.</div>';
        return;
      }
    } catch(_e) {}
    ...
    fetch('/api/auth/user', { headers: H })
      ...
      .then(function(me){
        if (!me) return;
        if (me.authority === 'TENANT_ADMIN' || me.authority === 'SYS_ADMIN') { renderWidget(host); return; }
        if (me.authority !== 'CUSTOMER_USER') return;
