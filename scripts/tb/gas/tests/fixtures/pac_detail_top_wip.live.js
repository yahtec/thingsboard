...
self.onDataUpdated=function(){
  var $c=self.ctx.$container; var host=$c.find('.pd-top'); if(!host.length) return;
    var RV=(function(){try{var v=sessionStorage.getItem('tduo.retroview.endTs');return v?parseInt(v,10):null;}catch(e){return null;}})();
    if(RV && self.__rvFetched===undefined){ self.__rvFetched=null; var __d0=(self.ctx.datasources&&self.ctx.datasources[0])||null; var __eid=__d0&&(__d0.entityId||(__d0.entity&&__d0.entity.id&&__d0.entity.id.id)); if(__eid){ fetch('/api/plugins/telemetry/DEVICE/'+__eid+'/values/timeseries?keys=pac_v2&startTs=0&endTs='+RV+'&limit=1&orderBy=DESC&agg=NONE',{headers:{'X-Authorization':'Bearer '+localStorage.getItem('jwt_token')}}).then(function(r){return r.json();}).then(function(d){ self.__rvFetched=(d&&d.pac_v2&&d.pac_v2.length)?d.pac_v2[0].value:null; self.onDataUpdated(); }).catch(function(){ self.onDataUpdated(); }); return; } }
  ...
  function getHP(){ var raw=null; if(typeof RV!=='undefined'&&RV){ raw=self.__rvFetched||null; } else { (self.ctx.data||[]).forEach(function(d){ if(d.dataKey&&/pac_v2/i.test(d.dataKey.name)&&d.data&&d.data.length) raw=d.data[d.data.length-1][1]; }); } var p=null; if(raw){ try{ p=(typeof raw==='string')?JSON.parse(raw):raw; }catch(e){} } var n=parseInt(stateParams().hpIndex||stateParams().hp||1)||1; return {n:n, H:(p&&p.HPs&&p.HPs[n-1])||{}}; }
  ...
  function gauge(val,min,max,col){
    ...
    var g=''; for(var i=0;i<=nn;i++){ var tt=i/nn; var p1=pt(tt,R),p2=pt(tt,R-9),pl=pt(tt,R-23);
      g+='<line x1="'+p1[0].toFixed(1)+'" y1="'+p1[1].toFixed(1)+'" x2="'+p2[0].toFixed(1)+'" y2="'+p2[1].toFixed(1)+'" stroke="#555" stroke-width="2"/>';g+='<text x="'+pl[0].toFixed(1)+'" y="'+(pl[1]+3).toFixed(1)+'" font-size="12" fill="#555" text-anchor="middle">'+Math.round(min+(max-min)*tt)+'</text>';
       }
    var tip=pt(t,R-6),tail=pt(t,-16); var c=col==='hp'?'#c92929':'#0288D1'; var bg=col==='hp'?'#fdeae8':'#e1f5fe';
    return '<svg viewBox="0 0 200 200" class="gz"><circle cx="100" cy="100" r="90" fill="'+bg+'" stroke="#ccc" stroke-width="2.5"/>'+g+
  ...
  var dataPac='<div class="pd-card pd-data"><div class="pd-card-t">Données PAC '+n+'</div><div class="pd-rows">'+
    ...
    kv('T° surchauffe',hp.tOH,' °C',1)+kv('Temps de fonctionnement',htime(hp.time),' h',0)+(hp.conR290!=null?kv('Concentration R290',hp.conR290*0.1,' %LFL',1)+'<div class="pd-kv"><span class="k">Capteur R290</span><span class="v">'+(hp.errR290==null?'—':(Number(hp.errR290)===0?'OK':String(hp.errR290)))+'</span></div>':'')+'</div></div>';
